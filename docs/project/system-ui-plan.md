# Skopos System UI Plan

Use this plan to shape the Skopos system UI, with a pilot-grade human console as the first delivery milestone before broader real-repo evaluations with people.

## Metadata

- Doc ID: `SKOPOS-PROJECT-SYSTEM-UI-PLAN`
- Status: `active`
- Owner: `skopos-core`
...
- `2026-04-13`: Implemented the first adapter-support UI slice, so `overview` now surfaces the generated enforcement adapter matrix from `.skopos/enforcement.json` and the product no longer hides Claude-versus-Codex lifecycle coverage behind internal tooling files.
- `2026-04-13`: Implemented the checkpoint-history follow-through for discussion context, so the routed console now loads `.skopos/discussions/index.json` plus recent checkpoint artifacts into console state, shows checkpoint history alongside the latest handoff in `overview` and `mission detail`, and indexes those checkpoints in the search dock while still deferring any dedicated discussion route.
...
- `2026-04-12`: Implemented the first routed workflow-state adoption slice, so the console now loads `.skopos/program/state.json`, shows compact program attention in `overview`, adds upstream program context to `mission detail`, exposes program pressure in `trust`, and adds search-dock jump targets for `do-now`, `do-next`, and open obligations.
- `2026-04-12`: Updated the system UI plan after the first low-noise program-router slice landed, so the next workflow-state UX batch now consumes a real `.skopos/program/state.json` artifact plus `skopos program sync` and `skopos program next` output instead of only planning around a future control-plane contract.
...
- `2026-04-11`: Updated the system UI plan to reflect the hardened `skopos ui dev` watcher scope after a self-hosted macOS watcher OOM, so generated app output under `docs/generated/skopos/app/**` and non-route-owned `.skopos/tooling/**` no longer participate in the live routed-console refresh loop.
...
- `2026-04-10`: Updated the system UI plan with the routed-app architecture, selected stack, delivery model, and phased migration away from the single-page static renderer as the primary pilot UI.
...
The primary pilot UI should now move from the current static HTML shell to a routed local app.
...
The routed console now needs to reflect the newer workflow surfaces as product state, not only as CLI or `.skopos` artifacts. The first compact program-control artifact now exists through `.skopos/program/state.json`, so the remaining UI batch should consume the implemented runtime state instead of inventing a parallel planning view.
...
Use a dual-surface approach during transition:

1. keep `skopos ui render` as the fallback snapshot renderer
2. add a routed local app as the primary pilot UI
3. build the routed app from compiled Skopos UI projections rather than direct ad hoc filesystem reads in components
...
2. `skopos ui serve`
3. `skopos ui dev`
4. keep `skopos ui render` during transition
...
2. `skopos ui serve`
3. `skopos ui dev`
4. compiled console-state artifacts written alongside the app bundle
...
7. `skopos ui render` still preserved as the snapshot fallback
...
The routed app remains a projection surface. `.skopos/**` and checked-in docs remain authoritative.
...
1. `.skopos/**` remains authoritative
2. `@skopos/ui` loaders compile route-ready view models
3. UI build emits app-ready state artifacts for the console
4. routed views consume typed projections for:
   - overview
   - mission detail
   - trust
   - proof
   ...
   - docs
   - activity
5. document projections should continue classifying support sections at build time, while the docs reader renders narrative sections through a real markdown component pipeline in the browser
...
6. `.skopos/**` state used by the current route set
...
6. generated app output or non-route-owned tooling churn changed:
...
1. `skopos ui dev`
   ...
   - watched compiled-state refresh for docs and `.skopos/**`
2. `skopos ui serve`
   - static preview and smoke-check lane
3. `skopos ui build`
   - output bundle generation
4. `skopos ui render`
   - fallback snapshot lane during transition
...
Do not throw away the existing shell immediately.

Transition in phases:

1. keep the current HTML renderer as fallback and snapshot output
2. the routed app shell and first top-level routes are now implemented
3. the real `ui dev` loop with UI HMR plus watched compiled-state refresh is now implemented
4. grouped left-rail navigation and first-class `plans`, `decisions`, and `findings` routes are now implemented
5. the first cleanup batch now lands on `overview`, `trust`, and `activity`, so those routes lead with human review surfaces instead of artifact-heavy summaries
...
5. Do not duplicate the source of truth away from `.skopos/**` and the checked-in docs.
...
4. keep `skopos ui dev` as the default contributor loop:
   ...
   - watched projection refresh for docs and `.skopos/**`
...
9. contributors can edit docs or `.skopos/**` state and see the routed console refresh without manually restarting the UI server
