---
title: P1-W11 Agent-Native Single Control Plane Convergence
status: superseded
owner: skopos-core
id: SKOPOS-P1-W11
scope: skopos
...
removalRule: move to archive during Phase 0 after active references point to the
---

# P1-W11 Agent-Native Single Control Plane Convergence

Historical prototype execution workpack. Its still-valid findings and proof inputs are
carried by the canonical product convergence Plan.

## Changelog

- `2026-07-28`: Superseded by the clean pre-release product contract. Compatibility
  migration, legacy readers, additive projections, permanent docs mapping, and
  worktree-first coordination are no longer target behavior.

- `2026-07-27`: Implemented the validation-economy slice: declared task-contract
  fields, mission revision/dirty baselines, explicit path adoption, affected package
  dependents, fail-fast checks, exact check receipts, and command-versus-action
  validation ownership. Eval now also re-resolves current workflow impact so stale
  admission-time actions cannot force unrelated validation, and done removes
  already-satisfied init, instruction, and workflow prompts.

- `2026-07-27`: Completed the real Unisane packed-install console proof: the generic
  CLI now serves bundled UI assets for installed `ui dev`, preserves source/HMR mode
  for Skopos development, refreshes live state without a second workflow authority,
  and passed fresh-install smoke plus the exact Unisane tarball pilot.
- `2026-07-27`: Opened the linked semantic-document-projection slice so alternate
  brownfield docs structures compile into the existing knowledge index for both agent
  retrieval and UI navigation without hardcoded adopter paths or a second authority.
...
1. `implemented`: compact task contract and brief
   ...
   - `start` accepts repeatable acceptance criteria, non-goals, constraints, and owned
     paths without inventing missing intent
   - existing mission state supplies declared goal, scope, decisions, and proof inputs
   - the compact brief exposes only genuinely missing task fields
...
3. `implemented`: phase-separated execution
   ...
   - mission admission records the starting Git revision and content state of
     pre-existing dirty paths
   - iteration and closure ignore unchanged pre-existing dirty files, retain task
     changes across commits, and include explicitly adopted paths
   - package impact selects affected packages plus transitive workspace dependents
   - exact successful command/source receipts survive phases until relevant state
     changes
   - validation stops at the first failure or timeout and records remaining checks as
     skipped
   - `validation.mode: actions` prevents duplicate inferred command checks when project
     workflows own validation
   - eval refreshes workflow requirements from current mission impact and reconciles
     workflow checklist items when registered action inputs change
...
9. `implemented`: compact artifact families, retention, and staged migration
   ...
   - no artifacts were deleted or silently relocated, no public command changed, and
     ...
     passes
10. `implemented`: cross-project adoption pilots
   ...
   - the matrix rejects any project whose compact lifecycle names a workflow authority
     other than Skopos
   - fixture proof establishes portability across isolated profiles
   - the live Unisane governed-monorepo pilot uses Skopos as the single workflow
     authority while preserving Unisane's authored domain docs, registered actions,
     and guards as project bindings
   - Unisane's generic document-projection binding compiles its hybrid docs structure
     into the same knowledge index and console without core path hardcoding
   - the exact packed CLI initialized Unisane, served the bundled live console, exposed
     11 of 11 project-knowledge roles, and reported 26 passing trust checks with no
     failures
   - the live console rendered Unisane decisions and semantic docs, filtered
     plans/workpacks, and produced no browser console errors
...
21. packed CLI release smoke installs the tarball into a fresh external project, builds
    the snapshot app, starts installed `ui dev`, and observes a live state refresh after
    project knowledge changes
22. exact Unisane pilot artifact
    `/tmp/skopos-unisane-pilot-v9/skopos-cli-0.1.0.tgz` has SHA-256
    `2aab424c93c1bddd624401571098eac4967c8b133c222749fdcafcc209e08e9b`
23. installed-package Unisane knowledge reports 11 of 11 roles mapped with no attention
    areas; trust reports 26 pass, 2 warn, and 0 fail
24. browser proof shows the bundled hashed app, Unisane semantic docs and decisions,
    working plan/workpack filtering, and no console errors
25. `@skopos/cli` source typecheck passes with the task contract, validation ownership,
    mission scope, impact, eval receipt, and compact closure changes
26. focused mission-scope and operating-model proof passes 10 tests, including
    unchanged pre-existing dirty isolation, explicit adoption, committed task changes,
    explicit workflow phases/risk lanes, and an intentionally empty closure check lane
27. focused CLI proof passes task-contract persistence, affected transitive dependents,
    action-owned zero-command validation, exact receipt reuse, fail-fast timeout
    behavior, and generated agent-contract guidance
28. the canonical instruction action completed after the source contract changed and
    regenerated Claude, Cursor, Copilot, and host-adapter projections from `AGENTS.md`
29. focused CLI proof passes current-impact workflow reconciliation: an action selected
    at admission is removed from eval and mission checklist state after its manifest no
    longer matches the mission-owned paths
30. focused CLI proof passes closure-action reconciliation: config refresh is requested
    only while bootstrap state is stale, synchronized mirrors are not requested again,
    and valid workflow receipts remove their run prompts from done
