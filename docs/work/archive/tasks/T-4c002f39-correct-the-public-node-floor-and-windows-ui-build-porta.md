...
# Task: Correct the public Node floor and Windows UI build portability
...
Correct the public Node floor and Windows UI build portability

## Acceptance

- The published CLI declares Node 22.13 or Node 24, matching unflagged node:sqlite availability and the certified matrix.
- The CLI UI-app build invokes pnpm through the active package-manager entrypoint and works across macOS, Linux, and Windows.
- Product UI Craft remains present in the packed CLI after the build-path repair.

## Non-Goals

- Add a SQLite fallback or certify Node 22.12 with an experimental runtime flag.
...
- `packages/cli/scripts/copy-ui-app.mjs`
...
- [x] **Implement the smallest scoped change** (implementation, complete) — Carry out "Correct the public Node floor and Windows UI build portability" inside the resolved scope before widening impact to adjacent areas.
...
- The published CLI declares Node 22.13 or Node 24, matching unflagged node:sqlite availability and the certified matrix. (closure, agent-observation)
- The CLI UI-app build invokes pnpm through the active package-manager entrypoint and works across macOS, Linux, and Windows. (closure, agent-observation)
- Product UI Craft remains present in the packed CLI after the build-path repair. (closure, agent-observation)
...
```json
{
  ...
  "proofSubject": {
    ...
    "baselineId": "baseline-f2e5dc5f4b34befa"
  },
  "priority": 0,
  "dependencyTaskIds": [],
  "steps": [
    ...
    },
    {
      ...
      "detail": "Carry out \"Correct the public Node floor and Windows UI build portability\" inside the resolved scope before widening impact to adjacent areas.",
      "status": "complete"
    },
    {
  ...
  "evidenceRequirements": [
    {
      "id": "acceptance-1",
      "acceptanceCriterion": "The published CLI declares Node 22.13 or Node 24, matching unflagged node:sqlite availability and the certified matrix.",
      "phase": "closure",
      "actionIds": [],
      "guardIds": [],
    ...
    {
      "id": "acceptance-2",
      "acceptanceCriterion": "The CLI UI-app build invokes pnpm through the active package-manager entrypoint and works across macOS, Linux, and Windows.",
      "phase": "closure",
      "actionIds": [],
      "guardIds": [],
      "evidence": "agent-observation"
    ...
    {
      "id": "acceptance-3",
      "acceptanceCriterion": "Product UI Craft remains present in the packed CLI after the build-path repair.",
      "phase": "closure",
      "actionIds": [],
      "guardIds": [],
      "evidence": "agent-observation"
  ...
  "memoryObligations": [
    {
      ...
      "resolutionReason": "The runtime floor and build-launcher portability correction does not alter Skopos architecture boundaries or component responsibilities; the release operations guide is the durable source that needs synchronization.",
      "resolvedAt": "2026-08-07T14:15:21.316Z",
  ...
  "questions": [
    {
      ...
      "resolvedAt": "2026-08-07T14:07:13.021Z",
      "resolvedByActorId": "codex"
    },
    {
      ...
      "resolvedAt": "2026-08-07T14:07:14.306Z",
      "resolvedByActorId": "codex"
    }
  ],
  "recommendations": [
