---
title: Archived Prototype Roadmap
status: historical
owner: skopos-core
id: SKOPOS-PLAN-P-37FA9180
scope: skopos
role: plan
...
# Archived Prototype Roadmap

> Historical prototype sequencing. The canonical convergence Plan is the only active
> roadmap authority.

## Changelog

- `2026-07-28`: Archived the superseded prototype roadmap with a collision-resistant
  Plan ID.
- `2026-07-28`: Replaced all immediate roadmap priority with the canonical clean
  pre-release convergence Plan. Historical phase descriptions below remain
  implementation inventory only and do not authorize compatibility or legacy growth.
...
- `2026-04-09`: Updated the roadmap to reflect that `.skopos/index/memory.json`, `.skopos/runs/operations.jsonl`, and the matching proof lane are now implemented, shifting the next proof work toward stronger freshness enforcement and multi-actor runtime pressure instead of index/log readiness.
...
The only current roadmap is the
[canonical product convergence Plan](../plans/P-e7e888e6-canonical-product-convergence.md):

1. canonical product reset
2. reproducible artifact ownership
3. canonical core model clean refactor
4. Project Memory standard
5. agent-guided adoption and restructuring
6. Profiles, Actions, and Guards
7. Tasks, context, and Work Queue
8. same-workspace Session coordination
9. host lifecycle, parity, and human UX
10. Unisane replacement pilot
11. release proof

No broader pack, UI, compatibility, or legacy-model work may bypass that order.

## Historical Prototype Focus
...
- Implement Memory Map v1 before more broad policy-pack expansion:
  ...
  - make `.skopos/index/roles.json` the durable runtime surface for observed, inferred, accepted, operational, and agent-ready memory
...
- Apply the token-control and compact-agent-transport contract before more control-plane growth:
  ...
  - add `.skopos/cache/agent/**` briefs for mission, program, trust, done, eval, and handoff state
...
- Keep the first low-noise program-control slice stable and expand it deliberately:
  - keep `.skopos/tasks/queue.json` as the single compact control-plane artifact
...
- Keep the mission-router baseline stable while the program-control layer is added:
  ...
  - keep `.skopos/tasks/questions.json` and `.skopos/tasks/recommendations.json` stable
  - keep router execution-surface guidance stable and default it to `artifact-only`
  - keep `.skopos/evidence/evals/*.json` stable
  ...
  - add the discussion-memory lane under `.skopos/sessions/` as the continuity input to the program router:
