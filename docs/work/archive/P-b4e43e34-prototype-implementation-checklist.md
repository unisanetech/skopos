---
title: Archived Prototype Implementation Checklist
status: historical
owner: skopos-core
id: SKOPOS-PLAN-P-B4E43E34
scope: skopos
role: plan
...
# Archived Prototype Implementation Checklist

> Historical implementation ledger. It does not authorize current work; the canonical
> convergence Plan and active Tasks own execution.

## Changelog

- `2026-07-28`: Archived this superseded prototype ledger with a collision-resistant
  Plan ID; active execution is owned by the canonical convergence Plan and Tasks.
- `2026-07-28`: The canonical convergence Plan now overrides the legacy “next slice”
  and build-order sections below. They remain current implementation inventory until
  Phase 0 archives or rewrites them; they are not target authority.
...
- `2026-06-24`: Added the first accepted-policy drift detection slice with `skopos policies drift`, `.skopos/index/policies/drift.json`, heuristic `architecture.mid-app` checks, policy-drift operational events, and trust integration for missing, stale, warning, and blocking drift reports.
...
- `2026-04-12`: Added the token-control and compact-agent-transport contract to the checklist, so the next implementation work now prioritizes compact command output, `.skopos/cache/agent/**` briefs, docs lifecycle filtering, lane minimization, and compact heavy-job polling before more control-plane growth.
- `2026-04-12`: Updated the checklist after the first routed program-state UI slice landed, so `overview`, `mission detail`, `trust`, and the search dock now consume `.skopos/tasks/queue.json` while the remaining workflow-state work shifts to questions/recommendations visibility and discussion-memory promotion instead of still treating basic UI adoption as pending.
- `2026-04-12`: Updated the checklist after the first low-noise program-router slice landed, so `.skopos/tasks/queue.json`, `skopos program sync`, and `skopos program next` are now implemented while the next structural work shifts to discussion-memory inputs and routed UI adoption instead of still treating the whole program lane as pending.
...
- `2026-04-12`: Updated the checklist with the accepted program-router and obligation contract, so the next structural slice now adds `.skopos/tasks/queue.json`, cross-mission sequencing, interruption guidance, and typed docs plus UI obligations above the implemented mission router instead of leaving that control plane in user memory.
...
- `2026-04-11`: Updated the checklist to reflect the hardened `skopos ui dev` watcher scope after a self-hosted macOS watcher OOM, so generated app output under `.skopos/ui/app/**` and non-route-owned `.skopos/cache/tooling/**` no longer participate in the live routed-console refresh loop.
...
- `2026-04-10`: Updated the checklist to reflect the durable `.skopos/evidence/proof/latest-report.json` proof artifact, so scorecard-versus-baseline output is kept as runtime-managed evaluation state instead of transient-only harness output.
...
- `2026-04-10`: Updated the checklist to reflect that `scan` now refreshes the durable diagnosis artifact, keeping brownfield diagnosis current in `.skopos/index/diagnosis.json` rather than only returning transient output.
...
- `2026-04-09`: Updated the checklist to reflect the implemented `.skopos/index/memory.json` and `.skopos/runs/operations.jsonl` artifacts plus the new proof lane that exercises them across the runtime lifecycle.
...
- `2026-04-09`: Updated the checklist to reflect the implemented local portal shell at `.skopos/ui/index.html`, with trust summary, artifact links, and graph entrypoints.
...
- `2026-04-09`: Updated the checklist to reflect that `skopos plan` now persists plan and mission artifacts under `.skopos/tasks/plans/` and `.skopos/tasks/missions/`.
- `2026-04-09`: Updated the checklist to reflect that messy-repo diagnosis and remediation reporting are now implemented through `skopos scan` and `.skopos/index/diagnosis.json`.
...
> **Convergence gate:** Do not admit broader product work before the canonical Plan's
> current phase exits. New source must use only the target vocabulary and must not add
> compatibility aliases, legacy readers, dual writes, permanent docs mapping, or
> worktree-only coordination.
...
7. Does it stay inside the accepted product decision and current convergence phase?
...
3. Generate `.skopos/index/roles.json` from observed, inferred, accepted, operational, and agent-ready project truth.
...
6. Generate `.skopos/cache/agent/communication-brief.json`.
...
7. Add the token-control transport layer before more control-plane surface growth:
   ...
   - `.skopos/cache/agent/**` brief artifacts
...
9. Add the policy-pack, stack-intelligence, and durable-memory foundation before creating a broad catalog:
   ...
   - drift evaluation against accepted policy is implemented through `skopos policies drift`
   - routed UI review for accepted packs, pack details, codebase verification signals, active rules, drift and execution lanes is implemented through the `Rules` route
   ...
   - trust and agent-brief integration for accepted policy posture is implemented; trust now reports policy drift posture; closure blocks open accepted `must` drift
...
10. Add the program router above the now-implemented mission router:
   - extend the first implemented `.skopos/tasks/queue.json` slice beyond active mission plus active findings
...
- Repo diagnosis and remediation reporting are implemented through `skopos scan` and `.skopos/index/diagnosis.json`.
...
- The UI package now renders a local HTML graph portal under `.skopos/ui/graph-portal.html`.
...
- A compiled `.skopos/index/architecture.json` artifact now exists with distinct current and recommended architecture views.
...
- A compiled `.skopos/index/enforcement.json` artifact now exists with generated Claude Code hook adapter outputs under `.skopos/cache/tooling/claude-code/`.
...
- A compiled `.skopos/index/memory.json` knowledge index and append-only `.skopos/runs/operations.jsonl` operational log now exist and are refreshed across init, plan, workflow, trust, impact, done, mission completion.
...
- Brownfield diagnosis now refreshes `.skopos/index/diagnosis.json` on `scan`, so the durable diagnosis artifact tracks the latest repo interpretation instead of only the last `init`.
- Query, trust, impact, and plan now prefer compiled `.skopos` state on hot paths and only refresh that state when tracked-source digest invalidation requires it.
...
- The proof harness now persists the latest scorecard and baseline comparison to `.skopos/evidence/proof/latest-report.json`.
...
- Closure now has an explicit accepted-policy gate: `skopos done` blocks when open accepted `must` drift remains in the resolved policy projection.
- The routed console now has a `Rules` page that projects accepted policy, full pack details, codebase verification signals, active rules, drift and execution-lane guidance from `.skopos/index/policies/**`, `policy-packs/**/pack.json`, and `.skopos/index/policies/drift.json`.

## Decision Gate

- The target product and clean-refactor boundary are resolved through:
  - `../../decisions/D-8d32a27b-canonical-project-memory-task-and-coordination-contract.md`
  - `../plans/P-e7e888e6-canonical-product-convergence.md`
...
- Allow the focused system UI batch described in `P-11229565-system-ui.md` once the proof scorecard remains passing; keep the first milestone constrained to pilot-readiness instead of broad surface expansion.
...
- Keep the user-facing surface simple:
  ...
  - one tracked Policy and Skill source
...
- Keep proof scorecard and baseline-comparison output durable on disk for self-hosting, but treat `.skopos/evidence/proof/**` as local runtime evaluation state rather than committed shared truth by default.
...
- Keep important shared runtime provenance visible to humans in the local portal instead of burying it only in raw `.skopos/runs/operations.jsonl` entries.
...
- Add generated docs only under `docs/reference/generated/`.
...
- Keep mutable Task and Session ownership explicit when more than one actor can touch the same workspace.
...
Follow the canonical convergence Plan in phase order:

1. finish canonical product reset and archive
2. make artifact ownership reproducible
3. cleanly replace the core model
4. implement the Project Memory standard and adoption restructuring
5. implement Profiles, Actions, Guards, Tasks, and Work Queue
6. implement same-workspace coordination
7. converge hosts and UI
8. replace Unisane's parallel workflow
9. pass release proof

## Historical Prototype Build Order

The numbered inventory below records the implemented prototype sequence. It is useful
for locating current source owners, but it does not authorize new Mission, Workpack,
Program, Workflow, Gate, Eval, Trust, compatibility, or mapping behavior.
