...
- `2026-08-05`: Closed after the canonical four-lane adoption matrix proved that
  assessment-only intake cannot report `agent-ready`, healthy greenfield and
  brownfield projects complete the reviewed lifecycle, and material ambiguity in a
  messy brownfield project blocks without mutation until an exact restructuring is
  reviewed, applied, verified, and activated. Session and the overview UI retain
  adoption orientation until activation. The proof runs through the canonical CLI
  `proof` command and is source-bound by Task T-db2a2a6c.
...
- Current State: fixed. Read-only assessment, reviewed proposal generation, shared
  Session decision delivery, exact risk-aware approval, coding-agent execution,
  standard verification, activation, Trust gating, and UI orientation are covered by
  one canonical greenfield/healthy-brownfield/messy-brownfield matrix.
...
2. ~~Show scanner limitations, provenance, contradictions, and required agent analysis
   through shared Session truth and the UI.~~
