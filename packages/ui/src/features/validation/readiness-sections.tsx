...
export function ReadinessInspectorAside({
  checkCount,
  warningCount,
  failureCount,
  findingCount,
...
export function ReadinessGuidanceCard({
  failureCount,
  warningCount,
  findingCount,
}: {
  ...
  return (
    ...
    >
      <div className="grid gap-3 md:grid-cols-3">
        ...
        <GuidancePoint
          label="Use when"
          text="You need to know if project state, docs, policy, and action checks are safe enough to continue."
        />
        <GuidancePoint
          label="Next step"
          text={
            hasAttention
              ...
              : 'Continue the active task and use Evidence when closure needs test proof.'
          }
        />
      </div>
    </Card>
...
export function ReadinessAttentionCard({
  failureChecks,
  warningChecks,
  findings,
    ...
    >
      <div className="grid gap-5">
        {failureChecks.length > 0 ? (
          <ReadinessCheckGroup title="Blocking checks" checks={failureChecks} tone="danger" />
        ) : null}
        {warningChecks.length > 0 ? (
          <ReadinessCheckGroup title="Warnings" checks={warningChecks} tone="warning" />
        ) : null}
        {findings.length > 0 ? (
