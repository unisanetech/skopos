...
export const renderProjectInstructions = ({
  ...
  return [
    ...
    '5. Use the Task returned by Session context. Inspect its bounded projection with `skopos task show <task-id> . --json`; add `--full` only when the complete portable state is required.',
    '6. Use `skopos instructions sync .` after changing `AGENTS.md` so mirrors and tool adapters stay aligned.',
    '7. After required Evidence exists, run `skopos finish <task-id> . --actor <id>` before claiming completion. It verifies, advances, archives, and rechecks final Readiness in one operation.',
    '',
  ].join('\n');
};
