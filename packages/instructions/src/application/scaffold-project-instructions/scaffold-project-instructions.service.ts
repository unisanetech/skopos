...
export const renderProjectInstructions = ({
  ...
  return [
    ...
    '5. Use the Task returned by Session context. Inspect its bounded projection with `skopos task show <task-id> . --compact --json` when more detail is needed.',
    '6. Use `skopos instructions sync .` after changing `AGENTS.md` so mirrors and tool adapters stay aligned.',
    '7. After required Evidence exists, run `skopos task verify <task-id> . --actor <id>`, then `skopos readiness <task-id> . --for close --advance --actor <id>` before claiming completion.',
    '',
  ].join('\n');
};
