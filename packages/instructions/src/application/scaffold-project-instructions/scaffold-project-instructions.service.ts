...
export const renderProjectInstructions = ({
  ...
  return [
    ...
    '5. Use `skopos task current . --actor <id> --json` during longer work to keep the current Task visible.',
    '6. Use `skopos instructions sync .` after changing `AGENTS.md` so mirrors and tool adapters stay aligned.',
    '7. Run `skopos verify <task-id> . --phase closure` and `skopos readiness <task-id> . --for close --actor <id>` before claiming completion.',
    '',
  ].join('\n');
};
