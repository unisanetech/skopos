...
const bindingPath = 'tools/skopos/skills/ui.product-craft.json';
const packPath = 'skill-packs/ui/product-craft/pack.json';
const suitePath = 'skill-packs/ui/product-craft/evaluations/core.suite.json';
const rubricPath = 'skill-packs/ui/product-craft/rubrics/product-ui-review.json';
const templateRoot = 'skill-packs/ui/product-craft/evaluations/templates';
const fixtureRoot = 'skill-packs/ui/product-craft/fixtures';
...
const runId = runIdArg?.slice('--run-id='.length) ??
  `product-ui-craft-${new Date().toISOString().replace(/[-:.]/g, '').slice(0, 15)}`;
...
const runPreflight = async (): Promise<RunReport['preflight']> => {
  ...
  await check('accepted-source-identity', async () => {
    const [evaluation, bindingSource] = await Promise.all([
      evaluateSkoposSkillFixturesRuntime({ cwd: skoposRoot, pack: 'ui.product-craft',
        binding: 'skopos.ui.product-craft', dryRun: true }),
    ...
    for (const [field, observed] of Object.entries(evaluation.artifact.identity)) {
      if (accepted[field] !== observed) throw new Error(`Accepted Product UI Craft identity is stale at ${field}.`);
    }
